...
void AppManager::setup() {
  ...
  singlePinDebug =
      new SinglePinDebug(m_serialShapeIOManager, 401, 356, 605, 605);
  ...
  waveModeContours =
      new WaveModeContours(m_serialShapeIOManager, kinectManager);
  ...
  for (map<string, Application *>::iterator iter = applications.begin();
       iter != applications.end(); iter++) {
...
void AppManager::setupShapeDisplayManagement() {
  ...
  heightsForShapeDisplay = std::vector<std::vector<unsigned char>>(
      m_serialShapeIOManager->shapeDisplaySizeX,
      std::vector<unsigned char>(m_serialShapeIOManager->shapeDisplaySizeY));
  heightsFromShapeDisplay = std::vector<std::vector<unsigned char>>(
      m_serialShapeIOManager->shapeDisplaySizeX,
      std::vector<unsigned char>(m_serialShapeIOManager->shapeDisplaySizeY));
  ...
  pinConfigsForShapeDisplay.resize(
      m_serialShapeIOManager->shapeDisplaySizeX,
      std::vector<PinConfigs>(m_serialShapeIOManager->shapeDisplaySizeY,
                              pinConfigs));

  // allocate height pixels objects and clear contents
  heightPixelsForShapeDisplay.allocate(
      m_serialShapeIOManager->shapeDisplaySizeX,
      m_serialShapeIOManager->shapeDisplaySizeY, 1);
...
void AppManager::update() {
  ...
  if (!paused) {
    ...
    pinConfigsAreStale = timeOfLastPinConfigsUpdate <
                         currentApplication->timeOfLastPinConfigsUpdate;
    if (pinConfigsAreStale) {
      currentApplication->getPinConfigsForShapeDisplay(
          pinConfigsForShapeDisplay);
    }
  }
...
void AppManager::draw() {
  ...
  int menuHeight = 350;
  string title =
      currentApplication->getName() + (showDebugGui ? " - Debug" : "");
  ofDrawBitmapString(title, menuLeftCoordinate, menuHeight);
  menuHeight += 30;
  ofDrawBitmapString(
      (string) "  '?' : " + (showGlobalGuiInstructions ? "hide" : "show") +
          " instructions",
      menuLeftCoordinate, menuHeight);
  if (showGlobalGuiInstructions) {
    menuHeight += 20;
    ofDrawBitmapString((string) "  '1' - '9' : select application",
                       menuLeftCoordinate, menuHeight);
    menuHeight += 20;
    ofDrawBitmapString((string) "  '.' : turn debug gui " +
                           (showDebugGui ? "off" : "on"),
                       menuLeftCoordinate, menuHeight);
    menuHeight += 20;
    ofDrawBitmapString((string) "  ' ' : " + (paused ? "play" : "pause"),
                       menuLeftCoordinate, menuHeight);
  }
  menuHeight += 30;
  ...
  if (!showDebugGui || currentApplication == applications["water"] ||
      currentApplication == applications["stretchy"]) {
    ofDrawBitmapString(currentApplication->appInstructionsText(),
                       menuLeftCoordinate, menuHeight);
    menuHeight += 20;
  }
  ...
  if (m_serialShapeIOManager->heightsFromShapeDisplayAvailable) {
    ...
    const auto &heightsFromBoards =
        m_serialShapeIOManager->getHeightsFromShapeDisplay();
  ...
  auto mouseGridPos = getMouseCoordinateInGrid(
      400, 50, 302, 302, m_serialShapeIOManager->shapeDisplaySizeX,
      m_serialShapeIOManager->shapeDisplaySizeY);
  if (!mouseGridPos.exists)
    mouseGridPos = getMouseCoordinateInGrid(
        705, 50, 302, 302, m_serialShapeIOManager->shapeDisplaySizeX,
        m_serialShapeIOManager->shapeDisplaySizeY);
  if (mouseGridPos.exists) {
    auto pos = mouseGridPos.coordinate;
    ofDrawBitmapString("Pixel Row: " + to_string(pos.second) +
                           "   Pixel Column: " + to_string(pos.first),
                       20, 20);
  }
